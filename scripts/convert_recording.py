...
def main():
    ...
    parser.add_argument("--montage", default=None, help="Standard montage name (e.g. standard_1020, GSN-HydroCel-128) to attach, producing electrodes.tsv + coordsystem.json. Use ONLY when the recording really used that layout and channel names match it. A generic cap layout is not a substitute for digitized positions: if you only know the scheme, leave this off and set EEGPlacementScheme via patch_sidecar.py instead. mne.channels.get_builtin_montages() lists the available names.")
